...
1. If you get an error about the model not being found:
   - Make sure you've run `train.py` first or
   - Specify the correct model path with `--model`

2. If no images are found:
   - Ensure the test directory contains image files
   - Check image format (supported: .jpg, .png, .jpeg)
   - Specify image paths directly with `--before` and `--after`

3. For GPU/CUDA issues:
   - The script automatically uses GPU if available
   - For CPU-only operation, no changes needed
